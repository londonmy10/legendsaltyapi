...
exports.run = (client, message, args, tools) => {
  ...
  if (
    ...
    const candyEmb = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setDescription(
        `\`${message.mentions.members.first().user.username}#${message.mentions.members.first().user.discriminator}\` ***seni*** \`${message.author.username}#${message.author.discriminator}\` ***öptü.*** `
      )
      .setImage(resimler);
