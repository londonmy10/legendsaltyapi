...
if(message.author.id !== message.guild.owner.user.id) return message.reply('Bu komut sunucu sahibine ozeldir!')

  if (!args[0]) {
message.channel.send("Lutfen **emojikur bilgi** kullaniniz.")
}
  ...
  if (args[0] === "bilgi") {
    message.channel.send(
      new Discord.RichEmbed()
        .setDescription("")
        .setColor("RANDOM")
        .setFooter(`${message.author.tag} Icin Istendi`, client.user.avatarURL)
        .setTimestamp()
        .addField(
          "**__EMOJIKUR HAZIRLIK__**",
          ` Emoji Kurulum Oncesi Mutlaka Bota Yonetici Yetkisi Veriniz.\nSunucunun Emoji Slotunun Bos Oldugundan Emin Olunuz.\nSonraki Asama Için emojikur kurulum Yaziniz.`
        )
    );
  }
  ...
  if (args[0] == "kurulum") {
    message.channel.send(
      new Discord.RichEmbed()
        ...
        .setTimestamp()
        .addField(
          "**__EMOJIKUR KURULUM ASAMASI__**",
          ` Emoji Kurulum Öncesi Asamalar Tamamlandi ise Komutu Giriniz;\n**emojikur baslat** `
        )
    );
  }
  ...
  if (args[0] == "baslat")
    message.channel.send(
      `Kurulum Baslatilsin mi? \n**evet** olarak cevaplayiniz...\n10 Saniye Sonra Iptal Edilir.`
    );
  message.channel
    ...
    .then(collected => {
      message.channel.send("Kurulum Baslatiliyor..");

      message.guild
        .createEmoji(`https://i.imgyukle.com/2020/05/05/riXHCn.gif`, "tik1")
        .then(tm => {
          message.channel.send("Emoji olusturuldu");
        })
        .catch(error => {
          message.channel.send("Hata olustu!\n\n" + error);
        })
        .then(Emoji =>
          message.guild
  ...
  message.channel.send("Tamamlandi")
  });
};
exports.conf = {
  enabled: true,
